...
import type { MenuTabType, RouteType } from '@/config/routes';
...
const { staticConf, dynamicConf } = extractRouteConfig(routeConfig);
export const staticConfig = staticConf;
export const dynamicConfig = dynamicConf;

function extractRouteConfig(rConfig: RouteType<MenuTabType[]>[]): {
  staticConf: RouteType<MenuTabType[]>[] | null;
  dynamicConf: MenuTabType[] | null;
} {
  if (!rConfig) return { staticConf: null, dynamicConf: null };

  let dc: MenuTabType[] | null = null;
  const copyConfig = (config: RouteType<MenuTabType[]>[]): RouteType<MenuTabType[] | boolean>[] =>
    config.map((conf) => {
      const route: RouteType<MenuTabType[] | boolean>[] = {
        ...conf
      };
      if (conf.children) {
  ...
  return { staticConf: copyConfig(rConfig), dynamicConf: dc };
}
...
// 翻译下route的Name
export function translateNameProperty(route: MenuTabType[]): MenuTabType[] {
  const newRoute: MenuTabType[] = [];
