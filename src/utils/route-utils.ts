...
function extractRouteConfig(rConfig: RouteType[]): {
  ...
  return { staticConf: copyConfig(rConfig), dynamicConf: dynamicConf.length ? dynamicConf : null };
}
...
): { staticRoute: RouteWithMenuType[] | null; menuTabs: DynamicRouteType[] | null } {
...
export const mergeRoute = (
  ...
  const replaceFlagPropertyByChilren = (array: RouteWithMenuType[]) => {
    return array.reduce((acc: RouteWithMenuType[], element: RouteWithMenuType) => {
      const item = {
        ...element,
