...
import { CUSTOM_NAV_PREFIX, ROOT_DOMAIN } from '@/config/base';
...
function debugLocal(data: any[]) {
  ...
  return data.map((item: any): any => {
    const { key, applicationUrl } = item;
    return { ...item, applicationUrl: temp[key.toLowerCase()] || applicationUrl };
  });
}
...
const getUrl = (appKey: string, url: string, isLogo?: boolean) => {
  const fa = '/favicon.png';
  const uri = url || `/${appKey}${isLogo ? fa : ''}`;
  const devUrl = isDev && !isUrl(url) ? `${ROOT_DOMAIN}${CUSTOM_NAV_PREFIX}${uri}` : uri;
  ...
  return request('/application/user/all').then((res) => {
    ...
    const replaceAppUrl = isDev
      ? (haveApplications ?? []).map((item: any) => {
          const { key, applicationUrl, logoUrl } = item;

          const nextAppUrl = getUrl(key, applicationUrl);
          const nextLogoUrl = getUrl(key, logoUrl, true);

          return { ...item, applicationUrl: nextAppUrl, logoUrl: nextLogoUrl };
        })
      : haveApplications;
